...
let id_rel t = `ID_Var(`Var_Constant,t)
...
let tok_to_string = function
  | L.K_CLASS _ -> "K_CLASS"
  | L.K_METACLASS _ -> "K_METACLASS"
  | L.K_INTERFACE _ -> "K_INTERFACE"
  ...
  | L.T_STRING(_,s) -> "T_STRING(" ^ s ^ ")"
  | L.K_END _ -> "K_END"
  | L.K_REQUIRE _ -> "K_REQUIRE"
  | L.K_ALIAS _ -> "K_ALIAS"
  | L.T_EOF -> "T_EOF"
...
let parse_annot entry str = 
  (*lextest (Lexing.from_string str);*)
  let lexbuf = Lexing.from_string str in
    entry TypeAnnotLexer.token lexbuf

let annot_test entry embed (desc,annot,ast) = 
  let d = desc ^ ": "  ^ annot in
    d >:: (fun () -> 
             let str = annot (*delim ^ (add_annot_prefix annot)*) in
	     let ast' = parse_annot entry str in
...
let class_test t = 
  let f (name,quants,cons) = ClassType((id_rel name),quants,cons) in
    annot_test TypeAnnotParser.e_class f t
let method_test t = annot_test TypeAnnotParser.e_method (fun x -> MethodType x) t

let expr_test (desc,annot,ast) = 
  let d = desc ^ ": "  ^ annot in
    d >:: (fun () -> 
	     let ast' = parse_annot TypeAnnotParser.e_expr annot in
               assert_equal ast' (ExprType ast)
                 ~cmp:equal_annotation
                 ~printer:string_of_annotation
	  )

let interface_test (desc,inter,annot) = 
  let test () = 
    let annot' = parse_annot TypeAnnotParser.interface_file inter in
      assert_equal annot' annot
        ~printer:string_of_interface
  in desc >:: test
...
let class_annot_tests = [
  ...
  ("absolute bounded quant class",
   "A<t>; t <= ::B",
   ("A",[QVar "t", Some (Type_Ident (`ID_UScope "B"))],[])
  );

  ("scoped relative bounded quant class",
   "A<t>; t <= B::C",
   ("A",[QVar "t", Some (Type_Ident (`ID_Scope(id_rel "B","C")))],[])
  );

  ("scoped absolute bounded quant class",
   "A<t>; t <= ::B::C",
   ("A",[QVar "t", Some (Type_Ident (`ID_Scope(`ID_UScope "B","C")))],[])
  );
  ...
  ("class with 3 bounded params",
   ...
   ("A",[(QVar "t1",Some (tid "U1"));
         (QVar "t2",Some (Type_Ident (`ID_UScope "U2")));
         (QVar "t3'",Some (Type_Ident (`ID_Scope(id_rel "U4","U3"))));
        ],[])
  );
...
(*
  ("scoped = method name",
   "A.foo= : () -> Fixnum",
   [`ID_Scope(id_rel "A","foo="),[], MethodSig([],None,tid "Fixnum")]
  );

  ("scoped aref",
   "A.\"[]\" : () -> Fixnum",
   [`ID_Scope(id_rel "A", "[]"), [], MethodSig([],None,tid "Fixnum")]
  );
*)
...
let klass name body = 
  ClassDef((id_rel name, [],[]),body)

(* helper function to test various elements within a single class
   definition *)
let just_class name body = ([], [klass name body])
...
let interface_tests = [
  ...
  ("class definition with Absolute Scope",
  "class ::A end",
  ([],[ClassDef((`ID_UScope("A"),[],[]),[])])
  );
  ...
  ("single empty class definition",
   "class A end",
   (just_class "A" [])
  );

  ("two adjacent class definitions",
  ...
  ("require followed by class definition",
   "require \"foo\" \n class A end",
   (["foo"],[klass "A" []])
  );

  ("constant definition",
   "class A X : Fixnum end",
   (just_class "A" [Const("X",tid "Fixnum")])
  );

  ("instance var definition",
   "class A @x : Fixnum end",
   (just_class "A" [IVar("@x",tid "Fixnum")])
  );

  ("class var definition",
   "class A @@x : Fixnum end",
   (just_class "A" [CVar("@@x",tid "Fixnum")])
  );

  ("global var definition",
   "class A $x : Fixnum end",
   (just_class "A" [GVar("$x",tid "Fixnum")])
  );

  ("class with single method",
   "class A foo : Fixnum -> String end",
   (just_class "A" [Method[(id_rel "foo",[],MethodSig([tid "Fixnum"],None,tid "String"))]])
  );

  ("class with intersection method",
   "class A foo: Fixnum -> String \n foo: String -> String \n end",
   (just_class "A" [ Method [
      (id_rel "foo",[],MethodSig([tid "Fixnum"],None,tid "String"));
      (id_rel "foo",[],MethodSig([tid "String"],None,tid "String"))
   ]]) 
  );

  ("class with two distinct methods",
   "class A foo: Fixnum -> String \n bar: String -> String \n end",
   (just_class "A" [
     Method [ (id_rel "foo",[],MethodSig([tid "Fixnum"],None,tid "String")) ];
     Method [ (id_rel "bar",[],MethodSig([tid "String"],None,tid "String")) ]
   ]) 
  );

  ("nested class definition",
   "class A \n class B \n end \n end",
   (just_class "A" [Class(klass "B" [])])
  );

  ("single empty module definition",
   "module A end",
   ([],[ModuleDef((id_rel "A",[],[]),[])])
  );

  ("single empty metaclass definition",
   "metaclass end",
   ([],[MetaClassDef[]])
  );

  ("metaclass with one method",
   "metaclass foo: () -> String end",
   ([],[MetaClassDef[Method[(id_rel "foo",[],MethodSig([],None,tid "String"))]]])
  );

  ("class with one intersection method and one regular method",
   "class A foo: Fixnum -> String \n foo: String -> String \n bar: String -> String end",
   (just_class "A" [
     Method [ (id_rel "foo",[],MethodSig([tid "Fixnum"],None,tid "String"));
              (id_rel "foo",[],MethodSig([tid "String"],None,tid "String"))
            ];
...
let suite = "Annotation Tests" >:::
  [ 
   ...
   "Interfaces" >::: List.map interface_test interface_tests;
  ]
