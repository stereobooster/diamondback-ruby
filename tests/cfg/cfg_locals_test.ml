...
let true_tests = [
  ...
  ("mcall",
   refactor_string_cfg "x = 2+3",
   "x"
  );

  ("yield",
   refactor_string_cfg "x = yield(2)",
   "x"
  );
...
let false_tests = [
  ...
  ("mdef",
   refactor_string_cfg "def f() x = 2+3 end",
   "x"
  );

  ("class",
   refactor_string_cfg "class A; x = 2+3 end",
   "x"
  );

  ("module",
   refactor_string_cfg "module M; x = 2+3 end",
   "x"
  );

  ("begin",
   refactor_string_cfg "BEGIN {x = 3}",
   "x"
  );

  ("end",
   refactor_string_cfg "END {x = 3}",
   "x"
  );
(*
  | Seq of stmt list
