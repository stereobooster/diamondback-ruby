...
let succ_scope_test (desc,pre,post) = 
  ...
  let d = desc ^ ": "  ^ pre in
    d >::
	 ...
	 let loader = File_loader.create [] in
	 ...
	 let cfg2 = Cfg_scope.resolve_scopes scoper (refactor_string post) in
	   assert_load_result_equal cfg1 cfg2
      )
let iface_scope_test (desc,pre,post) = 
  conf.error_raises_exc <- true;
  let d = desc ^ ": " ^ pre in
    d >::
      (fun () ->
    let loader = File_loader.create [] in
    let scoper = Cfg_scope.create loader in
    let process_iface str =
      let iface = TypeAnnotParser.interface_file TypeAnnotLexer.token (Lexing.from_string str) in
      let iface_cfg = Cfg.stmt_of_interface iface in
      Cfg_scope.resolve_scopes scoper (File_loader.Interface iface_cfg)
    in
    assert_load_result_equal (process_iface pre) (process_iface post)
  )
...
let diff_scope_test (desc,pre,post) = 
  ...
  let d = desc ^ ": "  ^ pre in
    d >::
	 ...
	 let cfg1 = Cfg_scope.resolve_scopes scoper (refactor_string pre) in
	 let cfg2 = Cfg_scope.resolve_scopes scoper (refactor_string post) in
	   assert_load_result_neq cfg1 cfg2
      )
