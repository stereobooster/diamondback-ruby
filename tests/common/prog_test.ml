...
module Make(PS : ProgSuite) = struct
  ...
  let nano_cfg () : Cfg.stmt * string = 
    ...
    let ast = parse_ruby_file file in
  ...
  let build_cfg loader file = 
    ...
    let cfg = 
      ...
      else File_loader.load_result_to_cfg (File_loader.load_file loader file)
    in
      cfg

  let process_file fname path = 
    PS.setup fname;
    let result = 
    try
      conf.error_raises_exc <- true;
      let loader = File_loader.create (path :: ["../lib"]) in
      let cfg0, name = 
  ...
  let test_fail file = 
    let desc = Printf.sprintf "expect fail file %s" file in
      desc >:: 
        (fun () -> 
          try ignore(process_file file PS.fail_dir); raise Failed
	   with 
  ...
  let test_succeed file = 
    let desc = Printf.sprintf "expect succeed file %s" file in
      desc >:: (fun () -> ignore(process_file file PS.success_dir))

  let tests_list = 
    [List.map test_succeed (PS.find_files PS.success_dir);
    List.map test_fail (PS.find_files PS.fail_dir);
    ]
