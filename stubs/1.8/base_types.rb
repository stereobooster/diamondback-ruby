...
class Thread # FIXME
  ...
  ##% fork<^args> : (^args) {(^args) -> ret} -> Thread
  ...
  ##% pass : () -> NilClass
  def Thread.pass() end
  ##% start<^args> : (^args) {(^args) -> ret} -> Thread
...
class Regexp ### DONE
  ...
  ##% "~" : () -> Integer
  def ~() end
  ##% compile : Regexp or String -> Regexp
  ##% compile : (String, String) -> Regexp
...
class IO
  ...
  ##% for_fd : (Integer, ?String) -> IO
  def IO.for_fd(fd, mode="r") end
  ##% foreach<t> : (String, ?String) {String -> t} -> NilClass
  ...
  ##% pipe : () -> Array<IO>
  def IO.pipe() end
  ##% popen : (String, ?String) -> IO
  ##% popen<t> : (String, ?String) {IO -> t} -> t
  def IO.popen(cmd_string, mode="r") end
  ##% read : ([to_str : () -> String], ?Fixnum, ?Fixnum) -> String
  def IO.read(name, length=99, offset=0)  end
  ##% readlines : (String, ?String) -> Array<String>
  def IO.readlines(name, sep=$/) end
  ##% select : (Array<IO>, ?Array<IO>, ?Array<IO>, ?Fixnum) -> Array
  def IO.select(read_array, write_array=[], error_array=[], timeout=0) end
  ##% sysopen : (String, ?String, ?String) -> Fixnum
...
class Dir
  ...
  def Dir.[](*rest) end
  ##% chdir : (?String) -> Fixnum
  ##% chdir<t> : (?String) {String -> t} -> t
  def Dir.chdir(str="$HOME") end
  ##% chroot : String -> Fixnum
  def Dir.chroot(p0) end
  ##% delete : String -> Fixnum
  def Dir.delete(p0) end
  ##% entries : String -> Array<String>
  def Dir.entries(p0) end
  ##% foreach<t> : (String) {String -> t} -> NilClass
  def Dir.foreach(p0) end
  ##% getwd : () -> String
  def Dir.getwd() end
  ##% glob : (Array<String>, ?Fixnum) -> Array<String>
  ##% glob : (String, ?Fixnum) -> Array<String>
  ##% glob<t> : (Array<String>, ?Fixnum) {String -> t} -> NilClass
  ##% glob<t> : (String, ?Fixnum) {String -> t} -> NilClass
  def Dir.glob(*rest) end 
  ##% mkdir : (String, ?Integer) -> Fixnum
  def Dir.mkdir(name, perm=42) end
  ##% open : String -> Dir
  ##% open<t> : (String) {Dir -> t} -> t
  def Dir.open(p0) end
  ##% pwd : () -> String
  def Dir.pwd() end
  ##% rmdir : String -> Fixnum
  def Dir.rmdir(p0) end
  ##% unlink : String -> Fixnum
  def Dir.unlink(p0) end
end

module Process
  module GID # FIXME
    ##% change_privilege : !FIXME -> !FIXME
    def GID.change_privilege(*) end
    ##% eid : !FIXME -> !FIXME
    def GID.eid(*) end
    ##% eid= : !FIXME -> !FIXME
    def GID.eid=(*) end
    ##% grant_privilege : !FIXME -> !FIXME
    def GID.grant_privilege(*) end
    ##% re_exchange : !FIXME -> !FIXME
    def GID.re_exchange(*) end
    ##% re_exchangeable? : !FIXME -> !FIXME
    def GID.re_exchangeable?(*) end
    ##% rid : !FIXME -> !FIXME
    def GID.rid(*) end
    ##% sid_available? : !FIXME -> !FIXME
  ...
  ##% getpriority : (Fixnum, Integer) -> Fixnum
  def Process.getpriority(p0, p1) end
  ##% getrlimit : Fixnum -> Array<Fixnum>
  ...
  ##% groups= : Array<Fixnum> -> Array<Fixnum>
  def Process.groups=(p0) end
  ##% initgroups : (String, Fixnum) -> Array<Fixnum>
  def Process.initgroups(p0, p1) end
  ##% kill : (Integer or String, Fixnum, *Fixnum) -> Fixnum
  ...
  ##% setpgid : (Fixnum, Integer) -> Fixnum
  def Process.setpgid(p0, p1) end
  ##% setpgrp : () -> Fixnum
  def Process.setpgrp() end
  ##% setpriority : (Fixnum, Integer, Fixnum) -> Fixnum
  def Process.setpriority(p0, p1, p2) end
  ##% setrlimit : (Fixnum, Fixnum, ?Fixnum) -> NilClass
  ...
  ##% wait : (?Fixnum, ?Fixnum) -> Fixnum
  def Process.wait(pid=-1, flags=0) end
  ##% wait2 : (?Fixnum, ?Fixnum) -> Array<Fixnum>
  def Process.wait2(pid=-1, flags=0) end
  ##% waitall : () -> Array<Array<Fixnum> >
  def Process.waitall() end
  ##% waitpid : (?Fixnum, ?Fixnum) -> Fixnum
  def Process.waitpid(pid=-1, flags=0) end
  ##% waitpid2 : (?Fixnum, ?Fixnum) -> Array<Fixnum>
...
class File < IO 
  ...
  ##% basename : ([to_str : () -> String], ?String) -> String
  def File.basename(file_name, suffix=".foo") end
  ##% blockdev? : String -> Boolean
  def File.blockdev?(p0) end
  ##% chardev? : String -> Boolean
  def File.chardev?(p0) end
  ##% chmod : (Integer, *String) -> Fixnum
  def File.chmod(mode, *files) end
  ##% chown : (Fixnum, Fixnum, *String) -> Integer
...
#  def File.foreach(*rest) end # TODO - from IO
  ...
  ##% grpowned? : String -> Boolean
  def File.grpowned?(p0) end
  ##% identical? : (String, String) -> Boolean
  def File.identical?(p0, p1) end
  ##% join : (*[to_str:() -> String]) -> String
  ##% join : (Array<[to_str:() -> String]>) -> String
  def File.join(*rest) end
  ##% lchmod : (Integer, *String) -> Integer
  def File.lchmod(mode, *files) end
  ##% lchown : (Integer, Integer, *String) -> Integer
  def File.lchown(owner_int, group_int, *files) end
  ##% link : (String, String) -> Fixnum
  def File.link(p0, p1) end
  ##% lstat : [to_str:() -> String] -> File::Stat # DAVIDAN
...
class Time
  ...
  ##% at : Time -> Time
  ##% at : (Numeric, ?Numeric) -> Time
  def Time.at(secs, micro=0) end
  ##% gm : (Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric, \
  ##%            ?Numeric) -> Time
  ##% gm : (Numeric, Numeric, Numeric, Numeric, Numeric, Numeric, \
  ...
  ##% local : (Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric,\
  ##%               ?Numeric) -> Time
  ##% local : (Numeric, Numeric, Numeric, Numeric, Numeric, Numeric, \
  ##%               Numeric, Numeric, Boolean, String) -> Time
  ...
  ##% mktime : (Numeric, Numeric, Numeric, Numeric, Numeric, Numeric, \
  ##%                Numeric) -> Time
...
#  def Time.times() end  # TODO deprecated
  ##% utc : (Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric, ?Numeric, \
  ##%             ?Numeric) -> Time
  ##% utc : (Numeric, Numeric, Numeric, Numeric, Numeric, Numeric, \
  ##%             Numeric, Numeric, Boolean, String) -> Time
...
module Marshal #FIXME
  MAJOR_VERSION = 4
  MINOR_VERSION = 8
  ##% dump<t> : (t,?Fixnum) -> String
  ##% dump<t,io> ; io <= BaseIO : (t,io,?Fixnum) -> io
  def Marshal.dump(*rest) end
  ### the proc is passed *every* (sub-)object that is deserialized
  ##% load<t> : (BaseIO or [to_str: () -> String],?Proc<^(Object),NilClass>) -> t
...
module YAML
  ...
  ##% load : String -> .?
  ##% load : [read : (?Fixnum, ?String) -> String] -> .?
  def YAML.load(*) end
  ##% load_documents<t> : (BaseIO) {.? -> t} -> t
  def YAML.load_documents(*) end 
  ##% quick_emit<t> : (Fixnum,?Hash) {YAML::Emitter -> t} -> String
...
module DRuby
  class Profile
    class Runtime
      ##% safe_require : (String,String) -> Boolean
      def self.safe_require(p1,p2) end
      ##% safe_load : (String,String,?Boolean) -> Boolean
      def self.safe_load(*) end
      
      ##% exn_require : (String) -> Boolean
      def self.exn_require(p1) end
      ##% exn_load : (String,?Boolean) -> Boolean
      def self.exn_load(s,b=false) end

      ##% dead_require : (*.?) -> Boolean
