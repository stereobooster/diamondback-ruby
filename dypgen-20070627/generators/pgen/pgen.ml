...
let a_start_def = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
...
let a_parser_param_info = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
...
let a_optional_code = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
    | [] -> Code ""
    | [Code c] -> Code c
...
let a_token_list = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
...
let a_relation = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
    | [Obj_void] -> Relation []
...
let a_grammar = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
...
let a_bar_opt = Dyp_special_types.Dypgen_action(fun _ _ _ d dd ld prd _ -> Obj_void,true,Data_void,dd,ld,[],[],prd,None,None)
...
let a_literal_list = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
    | [] -> Literal_list []
    | [Obj_uident tok] ->
...
let a_priority = Dyp_special_types.Dypgen_action(fun l _ _ d dd ld prd _ ->
  (match l with
...
] empty_priority_data `LR0 Data_void 0 P.Tools.empty_merge_map global_merge P.Tools.empty_datadyn nt_names
(Array.make 11 0)
...
let aux (lhs_nt,p,ld_list,ocaml_code) =
  ...
  let code_action = match ocaml_code with
    | Classic_action oc_code ->
        "Dyp_special_types.Dypgen_action(fun action_variable__l _ _ _data _dd _ld _prd _ -> (match action_variable__l with ["^
    ...
    | Full_action oc_code ->
        ...
        code_var_list^"] -> "^
        "let ob,b,d = "^oc_code^
        " in ((Obj_"^lhs_nt^" ob),b,d,_dd,_ld,[],[],_prd,None,None)\n"^
...
let code_main_2 = "let parsing_device = create_parsing_device rapf_list empty_priority_data `LR0 0 0 merge_map merge P.Tools.empty_datadyn nt_names (Array.make (Array.length nt_names) 0)\n"
...
"let "^non_terminal_start^" f lexbuf =
  ...
  " data_equal  [|fun _ -> true|] (fun _ -> \"\") f lexbuf lexbuf_position in
