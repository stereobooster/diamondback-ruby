...
let rec parse_ruby_lexbuf_with_state ?env state lexbuf = 
  try 
    NewParser.clear_env ();
...
and parse_string_with_state state ?env ?filename ?lineno str = 
  ...
  let line = match lineno with None -> 1 | Some x -> x in 
    ...
    parse_ruby_lexbuf_with_state ?env state lexbuf

let parse_ruby_lexbuf lexbuf = 
  let state = RubyLexerState.create NewLexer.top_lexer in 
    parse_ruby_lexbuf_with_state state lexbuf

let parse_interface_lexbuf lexbuf =
  (* TODO: Add error checks. *)  
  TypeAnnotParser.interface_file TypeAnnotLexer.token lexbuf


let parse_ruby_string ?env ?filename ?lineno str = 
  let state = RubyLexerState.create NewLexer.top_lexer in 
    parse_string_with_state state ?env ?filename ?lineno str

let parse_interface_string str =
  let lexbuf = Lexing.from_string str in
  parse_interface_lexbuf lexbuf

let parse_file fname parse_fun = 
  let ic = open_in fname in
  let lexbuf = Lexing.from_channel ic in
  let () = set_lexbuf_fname lexbuf fname in
    try
      let res = parse_fun lexbuf in
        close_in ic;
        res
    with e -> close_in ic; raise e

let parse_ruby_file fname = parse_file fname parse_ruby_lexbuf 

let parse_interface_file fname = parse_file fname parse_interface_lexbuf
