...
let to_ident x = (x:Ant.type_ident:>Cfg.identifier)
...
let load_file seen loader fname rl pos func = 
  try
    ...
    let full_name = src_name (find_file ~no_ext loader fname) in
      if StrSet.mem full_name seen then ()
      else 
        match load_file loader fname ~no_ext with
          | Ruby(cfg) 
          | Interface(cfg) -> func full_name cfg
          | Native(s) -> ()
  with Not_found -> ()
...
object(self)
  ...
  method private load_file pos arg rl =
    let visit_at_root f vtor x name =
    ...
    | `Lit_String fname ->
        load_file seen_files loader fname rl pos begin fun name stmt -> 
          visit_at_root visit_stmt (self:>cfg_visitor) stmt name;
        end 
    | e -> 
  ...
  method visit_stmt stmt = 
    ...
    | Cfg.Class(lhs_o, NominalClass(id,inh), body) ->
        let sc,str = NS.scope_of_identifier_split id in
...
object(self)
  ...
  method private in_env : 'a.('a -> 'a) -> 'a -> env -> 'a = fun f a new_env ->
    let old_env = env in
      env <- new_env;
      let ret = f a in
        env <- old_env;
  ...
  method private update_file pos env arg rl =
    let visit_at_root f vtor x name =
          seen_files <- StrSet.add name seen_files;
          let curs = NS.move_to_scope NS.root env.curs in
          let env' = {lex_scope=[NS.root];curs=curs} in
          self#in_env (f vtor) x env'
    in
    match arg with
    | `Lit_String id -> 
        let func_r = fun name stmt -> 
            let stmt' = visit_at_root visit_stmt (self:>cfg_visitor) stmt name in
            update_file loader name (Ruby stmt')
        in load_file seen_files loader id rl pos func_r
  ...
  method visit_stmt stmt = 
    ...
    match stmt.snode with
      ...
      | Cfg.Class(lhs_o,NominalClass(id,inh),body) ->
          ...
          let body' = self#in_env (visit_stmt (self:>cfg_visitor)) body env' in
            ...
            then SkipChildren
              ...
              let s' = update_stmt stmt (Cfg.Class(lhs_o',ck',body')) in
                ChangeTo s'
      ...
      | Cfg.Method(Singleton_Method(`ID_Self as id,msg),args,body) ->
          let curs = 
          ...
          let body' = self#in_env (visit_stmt (self:>cfg_visitor)) body env' in
            ...
            then SkipChildren
              ...
              let m = update_stmt stmt (Cfg.Method(sm,args',body')) in
                ChangeTo m
...
let resolve_scopes t l_result =
  ...
  in
   match l_result with
    | Ruby(stmt) -> Ruby(process stmt)
    | Interface(stmt) -> Interface(process stmt)
    | Native(s) -> Native(s)
