...
object(self)
  ...
  method visit_stmt stmt = 
    ...
    match stmt.snode with
	  ...
	  let (_:'a list) =  List.map (visit_method_param vtor') args in
            ignore (visit_stmt_children vtor' stmt);
            SkipChildren

        (* these start a new scope *)
      | Begin _ | End _  | Class _ | Module _  -> 
          let vtor' = ({<seen=StrSet.empty >} :> cfg_visitor) in
            ignore (visit_stmt_children vtor' stmt);
            SkipChildren
...
module Ant = Annotation

let mkannot (node : stmt_node) (annot : Ant.t option) =
  let st = mkstmt node Lexing.dummy_pos in
  match annot with
    | None -> st
    | Some a -> add_annotation st a

 (* FIXME
  * Because of intersections, method arity isn't plainly obvious, and can't
  * be easily infered from annotations. Right now, methods have empty bodies
  * and a single star argument. Is that going to work?  Does arity matter if
  * the method is annotated?
  *)
let args_of_annots meths = [`Formal_star "p0"] 

let list_to_seq lst f = mkannot ( Seq (List.map f lst) ) None

let rec stmt_of_interface (iface : Annotation.interface) = match iface with
  | (reqs, c_defs) ->
      mkannot (Seq((List.map stmt_of_require reqs) @ (List.map stmt_of_class_def c_defs))) None

(* Turn static requires into "require" method calls.
 * TODO What if interfaces require other interface files?
 *      Do we want them to? Is it a problem if they do?
 *)
and stmt_of_require (req : Annotation.require) =
  let mc = { mc_target  = None;
             mc_msg     = `ID_MethodName "require";
             mc_args    = [`Lit_String req];
             mc_cb      = None;
           }
  in
    mkannot (MethodCall(None,mc)) None

and stmt_of_class_def (c_def : Annotation.class_def) = match c_def with
  | Ant.ClassDef((t_id,_,subtypes) as annot, elems) ->
      let subtype = match subtypes with
        | Ant.Type_Ident(ht) :: [] -> Some (ht : Ant.type_ident :> identifier)
        | [] -> None
        | _ -> assert false
      in
      mkannot
        (Class(None, NominalClass((t_id : Ant.type_ident :> identifier), subtype),
               list_to_seq elems stmt_of_class_elem))
        (Some (Annotation.ClassType annot))
  | Ant.ModuleDef((t_id,_,_) as annot, elems) ->
      mkannot
        (Module(None, (t_id : Ant.type_ident :> identifier),
                list_to_seq elems stmt_of_class_elem))
        (Some (Annotation.ClassType annot))
  | Ant.MetaClassDef(elems) ->
      (* each metaclass definition gets represented as self.<name>, so
         don't enclose resulting cfg stmts in a metaclass block *)
      (list_to_seq elems (stmt_of_class_elem ~metaclass:true))

and stmt_of_class_elem ?(metaclass=false) (elem : Annotation.class_elem) =
  match elem with
  | Ant.Alias(id1, id2) -> assert false (* FIXME *)
  | Ant.Class(c_def)    -> stmt_of_class_def c_def
  | Ant.Method(meths)   -> 
      begin match meths with
      | (`ID_Var(`Var_Constant,name),_,_) :: _ ->
          let method_id =
            if metaclass then
              Singleton_Method(`ID_Self, `ID_MethodName name)
            else
              Instance_Method(`ID_MethodName name)
          in
          mkannot
            (Method(method_id, args_of_annots meths, empty_stmt ()))
            (Some (Annotation.MethodType meths))
      | _ -> assert false
      end
  | _ -> assert false
