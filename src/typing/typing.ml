...
let c_stub file = 
  ...
  let flat = Filename.concat conf.stub_dir fname in
    if Sys.file_exists flat then File_loader.RubyFile flat
    else 
      try try_each
...
let rec add_params_to_env env (params:any_formal list) ?block pos
  ...
  let rec add_varargs env lst = match lst with
    ...
    | `Formal_default(s,`Lit_Atom a)::ps when prefix "__rat_default_" a ->
  ...
  let ext_meth_t, env = match annot with
    ...
    | Some ann -> 
        if conf.use_annotations then begin
          (* If dynamic profiling is enabled, create a contract for this method. *)
          if conf.emit_runtime_casts && not (is_base_types env) then
            Contracts.emit_method_contract env.cur_class_name ann pos;
          (* Context is updated for logging purposes. *)
          let ctx = Log.in_ctx (Log.of_loc pos) "annotation for %s" mname in
          (* Returns an updated environment and a "method", which is simply
           * a record with a name and a "method_kind" of type RType.method_kind Variable.t. *)
          let annot_env, mt = 
            Annotations.mt_of_annotation env.cons env.annot_env ann ctx pos
          in
          (* Test for the presence of a usable (not intersection) annotation. 
           * If the annotation is usable and annotations are being checked,
           * add a constraint to the graph with the method instance generated
           * by instantiate_annotation. 
           * Regardless of success, update the environment. *)
          ...
          int_meth_t, env
  in 
  (* In all cases, add_meth_f is a partially applied KClass.add_method *)
  let () = add_meth_f ext_meth_t in
  let env = {env with cur_method=Some int_meth_t} in
  let env = 
    if annot = None then 
      type_stmt env body
    else if conf.check_annotations && body.snode <> (Return (Some `ID_Nil)) then
      type_stmt env body
    else env
  in
...
and load_file env arg rl pos = match arg with
  | `Lit_String id -> 
      ...
      let full_name = 
        try File_loader.src_name (File_loader.find_file ~no_ext env.loader id)
        with Not_found ->
      ...
      then env
        ...
        in
  let cfg = match l_result with
    | File_loader.Ruby(cfg) 
    | File_loader.Interface(cfg) -> cfg
    | _ -> assert false
  in
        let env = type_stmt env cfg in
          {env with
...
let type_in_env env name cfg = 
  let cfg = 
    ...
    then match Cfg_scope.resolve_scopes env.scoper (File_loader.Ruby(cfg)) with
      | File_loader.Ruby(cfg) -> cfg
      | _ -> assert false
    else cfg 
  in
...
let rec type_load_result env (l_result:File_loader.load_result) = match l_result with
  | File_loader.Ruby(cfg) -> type_stmt env cfg
  | File_loader.Interface(cfg) -> type_stmt env cfg
  | File_loader.Native(_) -> assert false 
