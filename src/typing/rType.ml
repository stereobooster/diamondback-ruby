...
module Annotations = struct
  ...
  let rec type_of_annot_expr cg env annot ~ctx ~pos = match annot with
    | Type_Named(_,te) -> type_of_annot_expr cg env te ~ctx ~pos
    | Type_Var v  ->
    ...
    | Type_Ident (`ID_Var (`Var_Constant,"NilClass")) -> KTyp.create ctx
  ...
  and refactor_method_list cg env (lst:method_annotation list) ~ctx ~pos = 
    let name = match lst with
      ...
      | (ident,_,_)::rest -> short_ident_name ident
    in
    let rec work lgamma meth_acc = function
      | [] -> lgamma, meth_acc
      | (mname,args,mt)::rest ->
	  let name' = short_ident_name mname in
  ...
  let class_module_annotation cg env (name,quants,subs) kind ctx pos = 
    ...
    let cls_t = KTyp.new_instance cg ~name:(short_ident_name name) class_t ctx in
