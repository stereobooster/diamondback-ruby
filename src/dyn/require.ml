       ...
       List.fold_left 
         (fun acc (is_req,file,really_load,libdirs) ->
            match really_load with
              ...
              | `A true -> begin try
                  let fqsrc = File_loader.find_in_libpath ~no_ext:(not is_req)
                  file libdirs in
                  let fqname = File_loader.src_name fqsrc in
                    (* skip .so files *)
                    if File_loader.is_native fqsrc then acc
                    else (fqname, Utils.normalize_filename fqname)::acc
                with Not_found ->
...
let lookup_file tmp_tbl is_req file is_load libdirs pos : string * string = 
  match is_load with
        ...
        begin  
          let src = 
            ...
            with Not_found -> Log.fatal Log.empty "didn't find %s in libpath %s"
              file (list_to_string id libdirs)
          in
          let norm = normalize_filename (File_loader.src_name src) in
            if File_loader.is_native src
            then norm, norm
            else try norm, Hashtbl.find tmp_tbl norm
            with Not_found -> 
              Log.fatal (Log.of_loc pos)
                "didn't find %s(%s) in tmp_tbl" file norm
        end
          
    | `B exn -> assert false (*file, ""*) (* require raised an exception at runtime *)
...
let rec instrument_expr tmp_tbl (req_map:t) expr = 
  ...
  let rec replace_require req_or_load arg pos =
    let filename = normalize_filename pos.Lexing.pos_fname in
