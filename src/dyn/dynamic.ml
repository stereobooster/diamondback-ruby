...
module Make(DL : AnalysisList) : sig
  ...
  let change_env_rubylib rubylib = 
    ...
    let _,pos = Array.fold_left
      (fun (i,acc) s -> 
         if prefix "RUBYLIB=" s then (i+1, Some i)
         else (i+1,acc)
      ) (0,None) env
    in match pos with
      | None -> 
  ...
  let append_require cfg file = 
    ...
    let ast = Parse_helper.parse_ruby_string code in
  ...
  let instrument_and_save loader t req_info = try
    ...
    let tmp_files = List.map
      (fun (orig_file, real_file, tmp_file, oc) ->
         ...
         in
         let cfg = File_loader.load_result_to_cfg (
           File_loader.load_file ~no_ext:true loader ~mod_ast_f:modf orig_file ) in
         ...
         let cfg = DL.instrument_cfg cfg in
           ...
           File_loader.update_file ~no_ext:true loader tmp_file (File_loader.Ruby(cfg));
  ...
  let profile_init ?(append=false) t components = 
    let buf = Buffer.create 32 in
      ...
      Buffer.add_string buf "])\n";
      Cfg_refactor.refactor_ast (Parse_helper.parse_ruby_string (Buffer.contents buf))
  ...
  let init loader fname = 
    let norm_file = Utils.normalize_filename fname in
  ...
  let transform_files loader yaml_func req_t src_file =
    let norm_file = normalize_filename src_file in
    ...
    let tmp_tbl = Hashtbl.create 127 in
      ...
      List.iter 
        (fun (orig_file,file) ->
           if not (File_loader.has_native_ext file) then
             let modf s = 
               let ast = Require.modify_requires tmp_tbl req_t s in
                 DL.transform_ast da_t ast
             in
             let cfg = File_loader.load_result_to_cfg (
                       File_loader.load_file ~no_ext:true loader ~mod_ast_f:modf file ) in 
             let cfg = DL.transform_cfg da_t cfg in
               File_loader.update_file ~no_ext:true loader file (File_loader.Ruby(cfg))
        ) all_files
  ...
  let profile loader src_fname = 
    ...
    let fake_loader = File_loader.create [] in
  ...
  let transform loader src_fname = 
    ...
    let cfg = File_loader.load_result_to_cfg
                (File_loader.load_file ~no_ext:true loader src_fname) in
      cfg
...
module Cons(X:DynamicAnalysis)(XS:AnalysisList) : AnalysisList = struct
  ...
  let lookup map key pos = 
    let fname = Utils.normalize_filename pos.Lexing.pos_fname in
